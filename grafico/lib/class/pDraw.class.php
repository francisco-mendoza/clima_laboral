 ...
 class pDraw
  {
   ...
   function drawLegend($X,$Y,$Format="")
    {
     ...
     $Data = $this->DataSet->getData();
	 if(isset($Data['Leyenda']) && !empty($Data['Leyenda'])){
	 	$Data["Series"] = $Data['Leyenda'];
	 }
         ...
         if ( $Mode == LEGEND_VERTICAL )
          {
           ...
           $Lines = preg_split("/\n/",$Serie["Description"]);
           $vY = $vY + max($this->FontSize*count($Lines),$IconAreaHeight) + 15;
          }
         elseif ( $Mode == LEGEND_HORIZONTAL )
          {
           $Lines = preg_split("/\n/",$Serie["Description"]);
     ...
     foreach($Data["Series"] as $SerieName => $Serie)
      {
       if(isset($Serie['mostrar']) && $Serie['mostrar']==false)
	       continue; 	
           ...
           if ( $Family == LEGEND_FAMILY_BOX )
            {
			 ...
			 if(!empty($Serie["tipo_icono"]) && $Serie["tipo_icono"]=='circulo'){
   ...
   function drawScale($Format="")
    {
	 $arrEscalaColor  = isset($Format["coloresEscala"]) ? $Format["coloresEscala"] : null;
     ...
     $EspacioLabelLineaX	= isset($Format["EspacioLabelLineaX"]) ? $Format["EspacioLabelLineaX"] : 0;     
     $EspacioLabelLineaY	= isset($Format["EspacioLabelLineaY"]) ? $Format["EspacioLabelLineaY"] : 0;	 
             ...
             for($i=0;$i<=$Parameters["Rows"];$i++)
              {
               ...
               if ( $this->isValidLabel($Value,$LastValue,$LabelingMethod,$ID,$LabelSkip) && !$RemoveXAxis)
                {
                 $Bounds    = $this->drawText($XPos,$YPos+$OuterTickWidth+$YLabelOffset+$EspacioLabelLineaX,$Value,array("Angle"=>$LabelRotation,"Align"=>$LabelAlign));
             ...
             for($i=0;$i<=$Parameters["Rows"];$i++)
              {
               ...
               if ( $this->isValidLabel($Value,$LastValue,$LabelingMethod,$ID,$LabelSkip) && !$RemoveXAxis)
                {
				 if(!empty($arrEscalaColor)){
                   $Bounds  = $this->drawText($XPos-$OuterTickWidth+$XLabelOffset,$YPos,$Value,array("Angle"=>$LabelRotation,"Align"=>$LabelAlign,"R"=>$arrEscalaColor[$ID-2]["R"],"G"=>$arrEscalaColor[$ID-2]["G"],"B"=>$arrEscalaColor[$ID-2]["B"]));				 	
				 } else {
                   $Bounds  = $this->drawText($XPos-$OuterTickWidth+$XLabelOffset,$YPos,$Value,array("Angle"=>$LabelRotation,"Align"=>$LabelAlign,"R"=>0,"G"=>0,"B"=>0));				 	
				 }               	
             ...
             for($i=0;$i<=$Parameters["Rows"];$i++)
              {
               ...
               if(!$RemoveYAxis){
                 $Bounds  = $this->drawText($XPos-$OuterTickWidth-2-$EspacioLabelLineaY,$YPos,$Value,array("Align"=>TEXT_ALIGN_MIDDLERIGHT));
   ...
   function drawPlotChart($Format=NULL)
    {
	 ...
	 $arrEscala		 = isset($Format["escala"]) ? $Format["escala"] : null ; 
           ...
           foreach($PosArray as $Key => $Y)
            {
			 ...
			 if(!empty($arrEscala)){
			   ...
			   foreach($arrEscala as $llave => $valores){
			     if($Serie["Data"][$Key] >= $valores['minimo'] && $Serie["Data"][$Key] <= $valores['maximo']){
			     	$strColorPunto = $valores['color_punto'];
					$arr = explode(",",$strColorPunto);	
			     	$R = $arr[0];
			     	$BorderR = $arr[0];					
			     	$G = $arr[1];					
			     	$BorderG = $arr[1];					
             ...
             if ($Y != VOID)
              {
               if ( $RecordImageMap ) { $this->addToImageMap("CIRCLE",floor($X).",".floor($Y).",".$SerieWeight,$this->toHTMLColor($R,$G,$B),$SerieDescription." N".utf8_decode("º")." ".($Key+1),$this->scaleFormat($Serie["Data"][$Key],$Mode,$Format,$Unit)); }
               ...
               if ( $Serie["Data"][$Key] == 0 )
                {
                 ...
                 if ( $RecordImageMap ) { 
                 	 	if( $OverrideColors!=NULL){
$this->addToImageMap("RECT",floor($X+$XOffset+$XSpace).",".floor($Y1-1).",".floor($X+$XOffset+$XSize-$XSpace).",".floor($Y1+1),$this->toHTMLColor($OverrideColors[$ID]["R"],$OverrideColors[$ID]["G"],$OverrideColors[$ID]["B"]),$SerieDescription,$this->scaleFormat($Serie["Data"][$Key],$Mode,$Format,$Unit));                 	 		
                 	 	}else{
...
$this->addToImageMap("RECT",floor($X+$XOffset+$XSpace).",".floor($Y1).",".floor($X+$XOffset+$XSize-$XSpace).",".floor($Y2),$this->toHTMLColor($OverrideColors[$ID]["R"],$OverrideColors[$ID]["G"],$OverrideColors[$ID]["B"]),$SerieDescription,$this->scaleFormat($Serie["Data"][$Key],$Mode,$Format,$Unit));                 	 		
                 	 	}else{
   ...
   function drawStackedBarChart($Format=NULL)
    {
	 ...
	 $forzarValores			= isset($Format["ForzarValores"]) ? $Format["ForzarValores"] : FALSE;
     $this->LastChartLayout = CHART_LAST_LAYOUT_STACKED;
           ...
           foreach($PosArray as $Key => $Height)
            {
             if ( $Height != VOID && $Serie["Data"][$Key] != 0 )
              {
               ...
               if($RecordImageMap && $Serie['mostrar']){
               
			    
				if($Serie['flag']){					    	
				  if($Serie["valores"][$Key]<0){ $Serie["valores"][$Key] = $Serie["valores"][$Key] *-1;}
				  $this->addToImageMap("RECT",floor($X+$XOffset).",".floor($Y1-$YSpaceUp+$YSpaceDown).",".floor($X+$XOffset+$XSize).",".floor($Y2),$this->toHTMLColor($R,$G,$B),$SerieDescription,$this->scaleFormat($Serie["valores"][$Key],$Mode,$Format,$Unit));			    	
			    }else{
				  if($Serie["Data"][$Key]<0){ $Serie["Data"][$Key] = $Serie["Data"][$Key] *-1;}
				  if(isset($Serie["valores"][$Key])){
					if($Serie["valores"][$Key]<0){
						$Serie["Data"][$Key] = $Serie["valores"][$Key] *-1;
					  
					}else{
						$Serie["Data"][$Key] = $Serie["valores"][$Key];						
					}				  	
				  }			    			    	
               ...
               if ( $Rounded )
               ...
               else
                {
                 ...
                 if ( isset($InnerColor) && $InnerColor != NULL ) { $RestoreShadow = $this->Shadow; $this->Shadow = FALSE; $this->drawRectangle(min($X+$XOffset+1,$X+$XOffset+$XSize),min($Y1-$YSpaceUp+$YSpaceDown,$Y2)+1,max($X+$XOffset+1,$X+$XOffset+$XSize)-1,max($Y1-$YSpaceUp+$YSpaceDown,$Y2)-1,$InnerColor); $this->Shadow = $RestoreShadow;}
               ...
               if ( $DisplayValues && $Serie['mostrar'])
                {
                 //echo "mostrar<br>"; 	
                 ...
                 if ( $DisplayOrientation == ORIENTATION_HORIZONTAL || $DisplayOrientation == ORIENTATION_AUTO )
                  {
                   //if ( $TxtHeight < $BarHeight && $TxtWidth < $BarWidth  )
                    if($forzarValores || ($TxtHeight < $BarHeight && $TxtWidth < $BarWidth))
                    {
                     if($Serie['flag']){
                     	$DisplayR = 0;
                     	$DisplayG = 0;
                     	$DisplayB = 0;												 
					   $valor = $Serie["valores"][$Key];
                     }else{
                     	$DisplayR = 255;
                     	$DisplayG = 255;
                     	$DisplayB = 255;          
					   $valor = $Serie["Data"][$Key];						           	
                     }	
					 if($forzarValores){
					 	if($valor != VOID){
					 	  $valor = number_format(($valor),2);
						  $this->drawText($XCenter,$YCenter,$valor,array("R"=>$DisplayR,"G"=>$DisplayG,"B"=>$DisplayB,"Align"=>TEXT_ALIGN_MIDDLEMIDDLE,"FontSize"=>$DisplaySize,"FontName"=>$DisplayFont));
					 	}
						//echo 	$valor.'<br>';
					 } else {
                          $this->drawText($XCenter,$YCenter,$this->scaleFormat($valor,$Mode,$Format,$Unit),array("R"=>$DisplayR,"G"=>$DisplayG,"B"=>$DisplayB,"Align"=>TEXT_ALIGN_MIDDLEMIDDLE,"FontSize"=>$DisplaySize,"FontName"=>$DisplayFont));					 		
					 }
                     ...
                     //$this->drawText($XCenter,$YCenter,$this->scaleFormat($valor,$Mode,$Format,$Unit),array("R"=>0,"G"=>0,"B"=>0,"Align"=>TEXT_ALIGN_MIDDLEMIDDLE,"FontSize"=>$DisplaySize,"FontName"=>$DisplayFont));
                     $Done = TRUE;
                    }
                  }
             ...
             $X = $X + $XStep;
            }//por cada posicion array
          }//fin orientacion
        }//fin is drawable and abcissa
      }//fin que recorre todas las series
    }//fin funcion
